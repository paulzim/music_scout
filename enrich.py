...
def build_user_prompt(allowed_genres: List[str], c: CandidateArtist) -> str:
    # Keep it short. Small models behave better.
    schema = (
        '{\n'
...
def _coerce_enrichment(data: Dict, allowed_genres: List[str], fallback_name: str) -> Dict:
    # Clamp to allowed genres (case-insensitive).
...
def enrich_candidate(
    ...
    # Best-effort JSON parse. Small models sometimes add stray text.
    # Try strict parse first, then try to extract the first JSON object.
    try:
        data = json.loads(raw)
    ...
    # Attempt to salvage JSON object inside the output
    try:
        start = raw.find("{")
        end = raw.rfind("}")
